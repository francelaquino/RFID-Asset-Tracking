...
var Tags = '';
@Component({
  selector: 'app-assigntoworkorder',
  templateUrl: './assigntoworkorder.component.html',
  styleUrls: ['./assigntoworkorder.component.scss'],
})
export class AssigntoworkorderComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.readerService.getReaderByService('Assigntowo').subscribe({
  ...
  onCheckOut() {
    ...
    if (!this.formGroup.valid && employee === '') {
      ...
      if (
        ...
        this.workorderService.SaveAssignment(this.formGroup.value).subscribe({
          //next: (r) => ,
          error: (e) => {
          ...
          complete: () => {
            ...
            this.messageService.add({
              severity: 'info',
              detail: 'Item(s) successully assigned',
            });
  ...
  onReadStart() {
    ...
    //       function () {
    //         $.getScript(root.readerAddress + '/signalr/hubs', function () {
    //           $.connection.hub.url = root.readerAddress + '/signalr';

    //           var chat = $.connection.ConnectionHub;

    //           chat.client.ListenToData = function (
    //             message: string,
    //             ant: string
    //           ) {
    //             // if (ant == 'Ant2') {
    ...
    //             var index = root.items.findIndex((x) => x.Tag === message);
    //             if (index < 0) {
    //               i.Readcount = 1;
    //               root.items.push(i);
    //             } else {
      ...
      this.tagService.searchTagByRfid(tag).subscribe({
        ...
        complete: () => {
          ...
          if (
            ...
            this.items[i].Status.indexOf('Checkout') >= 0
          ) {
