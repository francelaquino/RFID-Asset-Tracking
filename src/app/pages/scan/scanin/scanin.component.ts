...
@Component({
  selector: 'app-scanin',
  templateUrl: './scanin.component.html',
  styleUrls: ['./scanin.component.scss'],
})
export class ScaninComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.readerService.getReaderByService('Scanin').subscribe({
  ...
  onCheckIn() {
    ...
    if (!this.formGroup.valid && employee === '') {
      ...
      this.workorderService.WorkOrderCheckin(this.formGroup.value).subscribe({
        //next: (r) => ,
        error: (e) => {
          this.messageService.add({ severity: 'error', detail: e });
          this.loading = false;
  ...
  addRemoveItem = (tag: string, event: any) => {
    ...
    this.groupItems.map((t) => {
      ...
      if (
        (t.Status == 'Unrecognized' || t.Status.indexOf('Checkin') >= 0) &&
        t.IsSelected
      ) {
        this.enableSubmit = false;
      }
